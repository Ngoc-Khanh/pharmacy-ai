...
class GroqService:
    def __init__(self):
        ...
        if not groq_api_key or groq_api_key == "your_groq_api_key_here":
            logger.warning("GROQ_API_KEY không được tìm thấy hoặc chưa được cấu hình trong environment variables")
            self.client = None
            self.model = "qwen-qwq-32b"
            return
        
        # Khởi tạo client khi có API key hợp lệ
        try:
            # Sử dụng cách khởi tạo đơn giản cho phiên bản mới
            self.client = Groq(api_key=groq_api_key)
            self.model = "qwen-qwq-32b"
            logger.info("GroqService đã được khởi tạo thành công với Groq client")
            
            # Test connection để đảm bảo API key hợp lệ
            logger.info("Testing Groq connection...")
            
        except Exception as e:
            logger.error(f"Lỗi khởi tạo Groq client: {e}")
            logger.error("Có thể do: 1) API key không hợp lệ, 2) Kết nối mạng, 3) Quota đã hết")
            self.client = None
            self.model = "qwen-qwq-32b"

    async def analyze_symptoms(self, symptoms: str, patient_age: int = None, patient_gender: str = None) -> Tuple[Dict, bool]:
        """
        ...
        try:
            ...
            prompt = self._create_diagnosis_prompt(symptoms, patient_age, patient_gender)
            
            # Tạo seed ngẫu nhiên để đảm bảo phản hồi mới mỗi lần
            random_seed = int(time.time() * 1000) % 10000
            
            logger.info(f"Calling Groq API with seed: {random_seed}")
            ...
            response = self.client.chat.completions.create(
                ...
                max_tokens=2500,  # Tăng max_tokens để có phản hồi chi tiết hơn
                top_p=0.9         # Tăng top_p để cho phép nhiều lựa chọn từ ngữ hơn
            )
            
            logger.info("Received response from Groq API successfully")
            
            # Parse response
            result, is_fallback = self._parse_ai_response(response.choices[0].message.content)
    ...
    def _create_diagnosis_prompt(self, symptoms: str, patient_age: int = None, patient_gender: str = None) -> str:
    ...
    def _parse_ai_response(self, response_text: str) -> Tuple[Dict, bool]:
        ...
        try:
            ...
            # Validate structure
            required_fields = ["primary_diagnosis", "alternative_diagnoses", "general_advice", 
                             "overall_severity_level", "related_symptoms", "recommended_actions"]
            ...
            if not all(key in primary for key in ["diagnosis_name", "confidence_percentage", "description", "reasons"]):
            ...
            if len(alternatives) != 2:
                logger.warning(f"Expected 2 alternative diagnoses, got {len(alternatives)}")
                ...
                if len(alternatives) < 2:
                    alternatives.extend([{
                        ...
                        "reasons": ["Triệu chứng chưa rõ ràng", "Cần thêm thông tin", "Khuyến khích gặp bác sĩ"]
                    }] * (2 - len(alternatives)))
                else:
            ...
            if len(result["general_advice"]) < 3:
                result["general_advice"].extend(["Nghỉ ngơi đầy đủ", "Uống nhiều nước", "Theo dõi triệu chứng"][:3-len(result["general_advice"])])
            
            if len(result["related_symptoms"]) < 3:
                result["related_symptoms"].extend(["Mệt mỏi", "Khó chịu", "Lo lắng"][:3-len(result["related_symptoms"])])
            
            if len(result["recommended_actions"]) < 3:
                result["recommended_actions"].extend(["Tham khảo ý kiến bác sĩ", "Theo dõi triệu chứng", "Nghỉ ngơi"][:3-len(result["recommended_actions"])])
            
            logger.info("✅ Response validation successful - returning REAL AI response")
